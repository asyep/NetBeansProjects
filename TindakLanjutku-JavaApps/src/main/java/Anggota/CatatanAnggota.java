...
public class CatatanAnggota extends javax.swing.JFrame {
    ...
    public CatatanAnggota(String username, Integer IDAnggota) {
        this.username = username;
        this.IDAnggota = SessionManager.getInstance().getUserId();
        initComponents();
        tampilkanCatatanPJ();
    }
...
public void tampilkanCatatanPJ() {
    String sql = "SELECT ch.id_catatan, ch.tanggal, ch.isi_catatan, ch.dibuat_oleh, t.judul AS judul_tugas " +
                 "FROM catatan_hasil ch " +
                 "JOIN tugas t ON ch.id_tugas = t.id_tugas " +
                 "LEFT JOIN tugas_user tu ON tu.id_tugas = t.id_tugas " +
                 "WHERE t.id_user = ? OR tu.id_user = ?";

    try (Connection conn = Koneksi.configDB();
         PreparedStatement pst = conn.prepareStatement(sql)) {

        pst.setInt(1, IDAnggota);
        pst.setInt(2, IDAnggota);

        ResultSet rs = pst.executeQuery();

        // Ubah ke GridLayout seperti di LogCatatan.java
        JPanel containerDinamis = new JPanel();
        containerDinamis.setLayout(new GridLayout(0, 2, 15, 15)); // 2 kolom dengan spacing 15px
    ...
    private void catatanKerjaItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_catatanKerjaItemActionPerformed
        new CatatanAnggota(username, IDAnggota).setVisible(true);
        dispose();
    ...
    private void roundedButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_roundedButton1ActionPerformed
        new UploadCatatanAnggota(username).setVisible(true);
        dispose();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CatatanAnggota("username_default",1).setVisible(true);
            }
        });
    }
